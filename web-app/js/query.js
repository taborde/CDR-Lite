...
function adjustFormList() {
    $.ajax({
        type: 'GET',
        dataType: 'text', 
        url: '/cdrlite/query/getStudyCodeForFormList?caseId=' + $("#caseId").val() + (!isNaN($("#organization\\.id").val()) ? '&organization.id=' + $("#organization\\.id").val() : '')
    }).done(function(data) {
//        alert('data: ' + data)
        if (data == 'GTEX') {
...
function adjustCandidateFormList() {
    $.ajax({
        type: 'GET',
        dataType: 'text', 
        url: '/cdrlite/query/getStudyCodeForCandidateFormList?candidateId=' + $("#candidateId").val() + (!isNaN($("#organization\\.id").val()) ? '&organization.id=' + $("#organization\\.id").val() : '')
    }).done(function(data) {
        if (data == 'GTEX') {
            $(".gtexFormCheckBox").hide();
            $(".gtexFormCheckBox input:checkbox").attr('checked', false);
...
function adjustInterviewFormList() {
    $.ajax({
        type: 'GET',
        dataType: 'text', 
        url: '/cdrlite/query/getStudyCodeForInterviewFormList?interviewId=' + $("#interviewId").val()
    }).done(function(data) {
        if (data == 'BPVELSI') {
            $(".gtexFormCheckBox").hide();
            $(".gtexFormCheckBox input:checkbox").attr('checked', false);
...
$(document).ready(function() {
    ...
    $(".dueDatePicker .datePickerChangeTime").change(function() {
        ...
        $.ajax({
            type: 'POST',
            dataType: 'text', 
            url: '/cdrlite/query/updateDueDate?queryId=' + queryId + '&newDay=' + newDay + '&newMonth=' + newMonth + '&newYear=' + newYear
        }).done(function(data) {
            if (data != "ERROR") {
