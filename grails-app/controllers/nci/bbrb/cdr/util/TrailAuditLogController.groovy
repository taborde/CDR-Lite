...
import nci.bbrb.cdr.util.pogo.AuditLogEventUnitRecord
  ...
  //static allowedMethods = [delete: 'POST', save: 'POST', update: 'POST']
    ...
    def trailLog = {
        ...
        while (true)
        {
            if (!id)
            {
                errorMessage = 'Parameter value not found.'
                break;
            }
            ...
            while ((!object)&&(className.equalsIgnoreCase('caseQMSignature')))
            {
                ...
                def instances = nci.bbrb.cdr.datarecords.qm.CaseQMSignature.findByCaseRecord(caseRecordInstance)  
                if (!instances) break
            ...
            if (!object)
            {
                errorMessage = 'No audit log record can be found with className=' + className + ', objectId=' + objectId
                break
            }
...
//            }
    ...
    {
        if (className.equalsIgnoreCase('CaseQMSignature')) return 'nci.bbrb.cdr.datarecords.qm.CaseQMSignature'
        if (className.indexOf('.') > 0)
        {
            if (className.equals('nci.bbrb.cdr.forms.bpv.BpvClinicalDataEntry'))
            {
                return 'nci.bbrb.cdr.forms.bpv.clinicaldataentry.BpvClinicalDataEntry' 
            }
            if (className.equals('nci.bbrb.cdr.forms.bpv.TherapyRecord'))
            {
                return 'nci.bbrb.cdr.forms.bpv.clinicaldataentry.TherapyRecord' 
            }
        ...
        {
            String cs
            if (tempC.className.equals('nci.bbrb.cdr.forms.bpv.BpvClinicalDataEntry'))
            {
                cs = 'nci.bbrb.cdr.forms.bpv.clinicaldataentry.BpvClinicalDataEntry' 
            }
            else if (tempC.className.equals('nci.bbrb.cdr.forms.bpv.TherapyRecord'))
            {
                cs = 'nci.bbrb.cdr.forms.bpv.clinicaldataentry.TherapyRecord' 
            }
            else
            {
                cs = tempC.className
...
//        else if (object instanceof nci.bbrb.cdr.forms.gtex.crf.SurgicalMedication)
//        {
