...
function warnSpecimenIdMissMatch(){
    var validSpecimen = "^" + $("#caseRecordID").html() + "-[0-9][0-9][0-9]?\$";
...
function redirectToLogin(){
    //alert("iam here to logout")
    window.location = '/cdrlite/logout?url='+escape(document.URL);
}
...
function activateCahubDatePickers( datefield ){
    $("input.cahubDate").datepicker({
        ...
        showOn: 'both', 
        buttonImage: '/cdrlite/images/datepicker.gif', 
        buttonText: 'Date', 
...
function checkModification2() {
    ...
    if(changed == 'Y') {
     ...
     return confirm("The blood specimen data will be sent to CBR. Are you sure the data is correct?")
...
function fetchActivities(){
    ...
    $.ajax({
        type: 'GET',
        dataType: 'json', 
        url: '/cdrlite/activityEvent/retrieveEvent?max=' + numResults + '&callback=?'
    }).done(function(data) {
        if(data.length > 0){
            var message = "";
            var linkText;
                ...
                if ((deidentified == null)||(deidentified == false)) {
                    ...
                    if(data[i].activityType.code == 'SHIP' || data[i].activityType.code == 'SHIPPINGRECEIPT' || data[i].activityType.code == 'SHIPINSP' || data[i].activityType.code == 'SHIPRECPTDISC'){
                        if (data[i].caseId == 'N/A' || data[i].caseId == 'n/a') {
                            linkText = "<a href=\"/cdrlite/shippingEvent/showByCbrId/" + data[i].restEventId + "\">" + data[i].restEventId + "</a>";
                        } else {
                            linkText = "<a href=\"/cdrlite/shippingEvent/showByCbrIdAndCase/" + data[i].restEventId + "/" + data[i].caseId + "\">" + data[i].restEventId + "</a>";
                        }
                    }else if(data[i].activityType.code == 'UNUSEDKIT'){
                        linkText = "<a href=\"/cdrlite/shippingEvent/showByCbrId/" + data[i].restEventId + "\">" + data[i].restEventId + "</a>";
                    }else if(data[i].activityType.code == 'PROCESSEVT'){
                        linkText = "<a href=\"/cdrlite/processingEvent/showByCbrId/" + data[i].restEventId + "\">" + data[i].restEventId + "</a>";
                    }else if(data[i].activityType.code == 'CANDIDATE' || data[i].activityType.code == 'CASELINK'){
                        linkText = "<a href=\"/cdrlite/candidateRecord/view/" + data[i].additionalInfo2 + "\">" + data[i].additionalInfo1 + "</a>";
                        ...
                        linkText = "<a href=\"/cdrlite/query/show/" + data[i].additionalInfo1 + "\">" + data[i].additionalInfo1 + "</a>";
                    }else if(data[i].activityType.code == 'INTERVIEW'){
                        linkText = "<a href=\"/cdrlite/interviewRecord/show/" + data[i].additionalInfo2 + "\">" + data[i].additionalInfo1 + "</a>";
                    }else if(data[i].activityType.code == 'INTERVIEWSTATUS'){
                        linkText = "<a href=\"/cdrlite/interviewRecord/show/" + data[i].additionalInfo1.split(',')[1] + "\">" + data[i].additionalInfo1.split(',')[0] + "</a>";
                    }else if(data[i].activityType.code == 'PROCESSEVT'){
                        if (((sessionStudy == 'BPV')||(sessionStudy == ''))&&(!isLink)) linkText = "<b>" + data[i].caseId +  "</b>";  
                        else linkText = "<a href=\"/cdrlite/caseRecord/display/" + data[i].caseId + "\">" + data[i].caseId + "</a>";
                    }else{
                        if (((sessionStudy == 'BPV')||(sessionStudy == ''))&&(!isLink)) linkText = "<b>" + data[i].caseId +  "</b>";  
                        else linkText = "<a href=\"/cdrlite/caseRecord/display/" + data[i].caseId + "\">" + data[i].caseId + "</a>";
                    }  
            ...
            if($("#recentactivity").length != 0){
                ...
                $("#activityEvent").html(message + "</ul><a href=\"/cdrlite/activitycenter\" title=\"View more in Activity Center\">more ...</a>");
            }
    ...
    for(var i=0; i < data.length; i++){
        ...
        if ((deidentified == null)||(deidentified == false))
        {
            if(data[i].activityType.code == 'SHIP' || data[i].activityType.code == 'IMAGEREADY' || data[i].activityType.code == 'SHIPPINGRECEIPT' || data[i].activityType.code == 'SHIPRECPTDISC' || data[i].activityType.code == 'SHIPINSP'){
                if (data[i].caseId == 'N/A' || data[i].caseId == 'n/a') {
                    eventText = "<a href=\"/cdrlite/shippingEvent/showByCbrId/" + data[i].restEventId + "\">" + data[i].restEventId + "</a>";
                } else {
                    eventText = "<a href=\"/cdrlite/shippingEvent/showByCbrIdAndCase/" + data[i].restEventId + "/" + data[i].caseId + "\">" + data[i].restEventId + "</a>";
                }
            }
            ...
            if(data[i].activityType.code == 'PROCESSEVT'){
                eventText = "<a href=\"/cdrlite/processingEvent/showByCbrId/" + data[i].restEventId + "\">" + data[i].restEventId + "</a>";
            }            
            if(data[i].activityType.code == 'CANDIDATE' || data[i].activityType.code == 'CASELINK'){
                candidateText = "<a href=\"/cdrlite/candidateRecord/view/" + data[i].additionalInfo2 + "\">" + data[i].additionalInfo1 + "</a>";
            }
            if(data[i].activityType.code == 'QUERY'){
                queryText = "<a href=\"/cdrlite/query/show/" + data[i].additionalInfo1 + "\">" + data[i].additionalInfo1 + "</a>";
            }
            if(data[i].activityType.code == 'INTERVIEW'){
                interviewText = "<a href=\"/cdrlite/interviewRecord/show/" + data[i].additionalInfo2 + "\">" + data[i].additionalInfo1 + "</a>";
            }
            ...
            if(data[i].activityType.code == 'FILEUPLOAD' && !data[i].caseId && data[i].study){
                ...
                else caseText = "<a href=\"/cdrlite/caseRecord/display/" + data[i].caseId + "\">" + data[i].caseId + "</a>";
            }
        } else {
...
function addPrintLink(){
    if( $("body").hasClass("view") || $("body").hasClass("show") || $("body").hasClass("edit") || $("body").hasClass("create") || $("body").hasClass("display") || $("body").hasClass("report")){
        ...
        while (path != null)
        {
            ...
            if (service.toLowerCase() != 'cdrlite') break;
...
function createVocabDialog(vocid,title){
    ...
    $.ajax({
     url: '/cdrlite/deathCircumstances/vocabdetails?vocid=' + vocid,
     success:function(data){
